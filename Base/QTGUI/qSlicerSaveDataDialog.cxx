...
QString qSlicerFileNameItemDelegate::forceFileNameValidCharacters(const QString& filename)
{
  ...
  for (int i = 0; i < filename.size(); ++i)
    {
     /* if (QString(filename[i]).compare(QString::fromLocal8Bit("+")) == 0)
      {
  ...
  : QDialog(parentWidget)
{
  ...
  QPushButton* OkButton = this->ButtonBox->button(QDialogButtonBox::Save);
  QPushButton* CancelButton = this->ButtonBox->button(QDialogButtonBox::Cancel);
  OkButton->setText(tr("确 定"));
...
void qSlicerSaveDataDialogPrivate::saveSceneAsDataMrb()
{
    int sceneRow = this->findSceneRow();
    QComboBox* box = qobject_cast<QComboBox*>(
        this->FileWidget->cellWidget(sceneRow, FileFormatColumn));
...
bool qSlicerSaveDataDialog::exec(const qSlicerIO::IOProperties& readerProperties)
{
  ...
  for (int nb = 0; nb < fileWidgetRow; nb++)
  {
      ...
      QTableWidgetItem* nodeStatusItem = d->FileWidget->item(nb, 6);

      selectItem->setFlags(selectItem->flags() | Qt::ItemIsUserCheckable);
      selectItem->setCheckState(Qt::Unchecked);
      QComboBox* box = qobject_cast<QComboBox*>(
          d->FileWidget->cellWidget(nb, 1));
      QString extension = box->itemData(
          box->currentIndex()).toString();
      auto status = nodeStatusItem->text();
      /* SelectColumn = 0,
           FileNameColumn = 0,
           FileFormatColumn = 1,
      ...
      if (extension == ".nii" || extension == ".nii.gz") {
          ...
          QTableWidgetItem* fileNameItem = d->FileWidget->item(nb, 0);
          ctkPathLineEdit* fileDirectoryEdit = qobject_cast<ctkPathLineEdit*>(
              d->FileWidget->cellWidget(nb, 2));
          ...
          QDir directory = fileDirectoryEdit->currentPath();
          QFileInfo file = QFileInfo(directory, fileNameItem->text());
          vecFiles.push_back(fileNameItem->text().toStdString());
      }
      if ((extension == ".seg.nrrd" || extension == ".nrrd") && status == "Modified")
      {
          selectItem->setFlags(selectItem->flags() | Qt::ItemIsUserCheckable);
          selectItem->setCheckState(Qt::Checked);
          HadModified = true;
      ...
      //QString extension = box->itemData(
      //    box->currentIndex()).toString();
      //if (extension == QString(".vtk"))
      //{
      //    int index = box->count();
      //    box->setCurrentIndex(index - 1);
      //}
      //else if (extension == QString(".nrrd"))
      //{
      //    QTableWidgetItem* selectItem = d->FileWidget->item(nb, 0);
      //    selectItem->setFlags(selectItem->flags() | Qt::ItemIsUserCheckable);
      //    selectItem->setCheckState(Qt::Unchecked);
 ...
 /* if (d->exec() != QDialog::Accepted)
  ...
  return true;*/
  ////d->saveSceneAsDataMrb();

  if (!HadModified)
  {
      QMessageBox::information(NULL, "提示", "没有修改文件！",
          QMessageBox::Yes);
      return false;
  }
  ...
  QString SaveFileName  = SavePath + "/save.zip";
  //kyo 保存完成打包压缩
  auto res = vtkArchive::VeiwerZip(SaveFileName.toStdString().c_str(), SavePath.toStdString().c_str(), vecFiles);
  if (!res)
  {
      return res;
  }
  ...
  sit = vecFiles.begin();
  while (sit != vecFiles.end())
  {
      auto stringfileName = SavePath.toStdString() + std::string("/") + (*sit);
      QFile fileTemp(QString::fromStdString(stringfileName));
      auto b = fileTemp.remove();
      sit++;
  }
...
bool qSlicerSaveDataDialogPrivate::confirmOverwrite(const QString& filepath)
{
  ...
  //    tr("文件: %1 已存在。 您要更换吗？").arg(filepath),
