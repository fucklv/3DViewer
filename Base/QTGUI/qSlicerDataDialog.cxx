    ...
    :QDialog(_parent)
{
    ...
    //kyo
    this->AddDirectoryButton->setVisible(false);
    ...
    resetButton->setText(tr("重 置"));
    CancelButton->setText(tr("取 消"));

    OkButton->setStyleSheet("QPushButton{font: 10pt 'SimHei';width: 68; height: 24;background: #191925; border: 1px solid #5c5c74;}QPushButton:hover{background-color: #5C5C74;}");
    resetButton->setStyleSheet("QPushButton{font: 10pt 'SimHei';width: 68; height: 24;background: #191925; border: 1px solid #5c5c74;}QPushButton:hover{background-color: #5C5C74;}");
    CancelButton->setStyleSheet("QPushButton{font: 10pt 'SimHei';width: 68; height: 24;background: #191925; border: 1px solid #5c5c74;}QPushButton:hover{background-color: #5C5C74;}");
...
bool qSlicerDataDialogPrivate::checkAndHandleArchive(const QFileInfo& file)
{
    ...
    {
      /*  if (QMessageBox::question(this, tr("打开存档？"),
                                  tr("所选文件是.zip存档文件，打开它并加载内容？")) == QMessageBox::Yes)
        {
...
bool qSlicerDataDialog::exec(const qSlicerIO::IOProperties& readerProperties)
{
    ...
    bool res = false;
    QStringList list = RecordInformationTool::getInstance()->getOpenFilePathList();
    if (list.size() == 0)
    {
        d->reset();
    ...
    else
    {
        ...
        res = openFilePaths(list, readerProperties);
        RecordInformationTool::getInstance()->clearFilesList();
...
bool qSlicerDataDialog::openFilePaths(const QStringList &list, const qSlicerIO::IOProperties& readerProperties)
{
    Q_D(qSlicerDataDialog);
