  ...
  : q_ptr(&object)
{
  ...
  //this->SegmentEditButton = nullptr;
}
...
void qSlicerModuleSelectorToolBarPrivate::init()
{
  ...
  // Modules comboBox
  this->ModulesComboBox = new ctkMenuComboBox();
  this->ModulesComboBox->setToolTip(qSlicerModuleSelectorToolBar::tr("从模块列表中选择一个模块"));
  ...
  this->PreviousButton->setToolTip(qSlicerModuleSelectorToolBar::tr("向前导航"));
  ...
  this->NextButton->setToolTip(qSlicerModuleSelectorToolBar::tr("向后导航"));
  ...
  //this->SegmentEditButton = new QToolButton(q);
  //const QIcon searchIcon = QIcon::fromTheme("edit-find", QPixmap(":/Icons/Icons/SegmentEditor.png"));
  //this->SegmentEditButton->setIcon(searchIcon);
  //this->SegmentEditButton->setText(qSlicerModuleSelectorToolBar::tr("SegmentEditor"));
  //this->SegmentEditButton->setToolTip(qSlicerModuleSelectorToolBar::tr("Segment Editor"));
  //QObject::connect(this->SegmentEditButton, SIGNAL(clicked(bool)),
  //    q, SLOT(slotSegmentEditToolButton()));
  //q->addWidget(this->SegmentEditButton);
  //QObject::connect(q, SIGNAL(toolButtonStyleChanged(Qt::ToolButtonStyle)),
  //    this->SegmentEditButton, SLOT(setToolButtonStyle(Qt::ToolButtonStyle)));
}
...
/*!
 * \brief SegmentEditor按钮 cxc
 */
void qSlicerModuleSelectorToolBar::slotSegmentEditToolButton()
{
    Q_D(qSlicerModuleSelectorToolBar);

    QString moduleName = "SegmentEditor";
    if (d->ModulesMenu->currentModule() == moduleName)
    {
        return;
    }

    d->ModulesMenu->setCurrentModule("SegmentEditor");
}

/*!
 * \brief 自定义工具按钮 cxc
 */
void qSlicerModuleSelectorToolBar::currentModuleToolButton(const QString& moduleName)
{
    Q_D(qSlicerModuleSelectorToolBar);
...
//Select a module by title. It looks for the module action and triggers it.(查找标题)
void qSlicerModuleSelectorToolBar::currentModuleToolButtonTitle(const QString& moduleNameTitle)
{
    Q_D(qSlicerModuleSelectorToolBar);

    if (d->ModulesMenu->currentModule() == moduleNameTitle)
    {
        return;
    }

    d->ModulesMenu->setCurrentModuleByTitle(moduleNameTitle);
}
