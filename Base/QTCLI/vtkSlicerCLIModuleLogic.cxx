...
void vtkSlicerCLIModuleLogic::Apply ( vtkMRMLCommandLineModuleNode* node, bool updateDisplay )
{
  ...
  if ( node->GetModuleDescription().GetType() == "PythonModule" )
    ...
    //Ryu=======================================================================
    auto sFlag = node->GetParameterValue(0, 4);
    if (!sFlag.compare("true"))
    {
        auto nodes = this->GetMRMLScene()->GetNodesByClass("vtkMRMLModelNode");
        vtkMRMLNode* ModelNode = nullptr;
        vtkCollectionSimpleIterator it;
        std::list<vtkMRMLNode*> pNodes;
        for (nodes->InitTraversal(it); (ModelNode = static_cast<vtkMRMLNode*>(
            nodes->GetNextItemAsObject(it)));)
        {
            auto kk = ModelNode->GetName();
            if (strcmp(kk, "Red Volume Slice") && strcmp(kk, "Yellow Volume Slice") && strcmp(kk, "Green Volume Slice"))
            {
                QString inputName = "inputModel";
                node->SetParameterAsNode(inputName.toUtf8(), ModelNode);
                QString outputModel = "outputModel";
                node->SetParameterAsNode(outputModel.toUtf8(), ModelNode);
                break;
            }
        }
