...
void qSlicerCLIModuleWidgetPrivate::setupUi(qSlicerWidget* widget)
{
  ...
  this->AutoRunPushButton->setVisible(false);
  this->ParameterSetLabel->setVisible(false);
  this->MRMLCommandLineModuleNodeSelector->setVisible(false);
  //Ryu
  if (title == QString::fromLocal8Bit("Decimation"))
  {
      this->DefaultPushButton->hide();
      this->label_2->setText("\346\250\241\345\236\213\345\207\217\351\235\242");
      this->label->setStyleSheet("QLabel{width: 32;height: 32;border-image: url(:/Icons/Icons/Remove surface.png);}");
  }
                                                     ...
                                                     const ModuleParameterGroup& parameterGroup)
{
  ...
  vbox->setHorizontalSpacing(8);
  collapsibleWidget->setLayout(vbox);
  collapsibleWidget->setStyleSheet("QAbstractButton{border: none;background: #28293a;}");
  ...
  // 屏蔽Advanced
  //-----------------------------------------------------------------------------
  if (collapsibleWidget->text() == "Advanced")
  {
                                               ...
                                               const ModuleParameter& moduleParameter)
{
  ...
  //-----------------------------------------------------------------------------
  // 屏蔽Anisotropic Diffusion Parameter选项
  //-----------------------------------------------------------------------------
  if (_label == "Conductance" || _label == "Time Step")
  {
      return;
  }
  ...
  //// 屏蔽Dcimation的 method选项
  ////-----------------------------------------------------------------------------
  //if (moduleParameter.GetName() =="outputModel")
  //{
  //    qMRMLNodeComboBox* combox = static_cast<qMRMLNodeComboBox*>(widget);
  //}
  //-----------------------------------------------------------------------------
  // 屏蔽Anisotropic Diffusion  IO的 Input Volume选项
  //-----------------------------------------------------------------------------
  if (_label == "Input Volume")
  {
      qMRMLNodeComboBox* combox = static_cast<qMRMLNodeComboBox*>(widget);
      combox->setRenameEnabled(false);
      combox->setRemoveEnabled(false);
  }
...
void qSlicerCLIModuleWidget::on_CreateParameterSetButton_clicked()
{
    Q_D(qSlicerCLIModuleWidget);

    d->MRMLCommandLineModuleNodeSelector->createNewAndAsNode("Create new Parameter set");
}

void qSlicerCLIModuleWidget::on_DeleteCurrentParameterSetButton_clicked()
{
    Q_D(qSlicerCLIModuleWidget);

    d->MRMLCommandLineModuleNodeSelector->removeCurrentNode();
}
...
void qSlicerCLIModuleWidget::onStateChanged(int state)
{
    //获取所有model node路径
    if (state) {
        Q_D(qSlicerCLIModuleWidget);
        //auto tt = d->CommandLineModuleNode->GetModuleDescription().GetParameterGroups()[0].GetParameters()[5];
        auto node = this->currentCommandLineModuleNode();
        QString filePath = QCoreApplication::applicationDirPath();
        const QString SAVE_PATH = "/Save";
        filePath = filePath + SAVE_PATH;
        node->SetParameterAsString("SavePath", filePath.toStdString());
        /*auto nodes = this->mrmlScene()->GetNodesByClass("vtkMRMLModelNode");
        vtkMRMLNode* node = nullptr;
        vtkCollectionSimpleIterator it;
        std::list<vtkMRMLNode*> pNodes;
        for (nodes->InitTraversal(it); (node = static_cast<vtkMRMLNode*>(
            nodes->GetNextItemAsObject(it)));)
        {
            auto kk = node->GetName();
            qDebug() << "Unknown widget value type:" << state;
            if (strcmp(kk,"Red Volume Slice")&& strcmp(kk, "Yellow Volume Slice")&& strcmp(kk, "Green Volume Slice"))
            {
    ...
    {
        auto node = this->currentCommandLineModuleNode();
        node->SetParameterAsString("SavePath", "models");
    }
}