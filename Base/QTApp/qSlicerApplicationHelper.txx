...
void splashMessage(QScopedPointer<QSplashScreen>& splashScreen, const QString& message)
{
  ...
  splashScreen->showMessage(message, Qt::AlignBottom | Qt::AlignHCenter, Qt::white);
}
...
// See details at https://issues.slicer.org/view.php?id=4252
  ...
  if (showSplashScreen)
    ...
    //splashMessage(splashScreen, "V1.0\n\n\n正在加载中.  ");
    splashScreen->show();
    }
  ...
  foreach(const QString& name, moduleFactoryManager->instantiatedModuleNames())
    ...
    //splashMessage(splashScreen, "加载模块 \"" + name + "\"...");
    //splashMessage(splashScreen, "V1.0\n\n\n正在加载中...");
  ...
  if (window)
    ...
    window->setViewLayout();
    window->showMaximized();
    //window->setFixedSize(1550, 900);

    QStringList list;
    //list << "D:/Desktop/nrrdfile/MRHead.nrrd";
    //RecordInformationTool::getInstance()->setOpenFilePathList(list);
