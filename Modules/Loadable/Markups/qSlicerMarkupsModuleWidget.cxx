...
qSlicerMarkupsModuleWidgetPrivate::qSlicerMarkupsModuleWidgetPrivate(qSlicerMarkupsModuleWidget& object)
  : q_ptr(&object)
{
  this->columnLabels << "Selected" << "Locked" << "Visible" << "名称" << "描述" << "R" << "A" << "S";
...
void qSlicerMarkupsModuleWidgetPrivate::setupUi(qSlicerWidget* widget)
{
  ...
  this->createPlanePushButton->setVisible(false);
  this->controlPointsCollapsibleButton->setVisible(false);
  this->CollapsibleGroupBox->setVisible(false);
  this->createFiducialPushButton->setVisible(false);
  this->angleMeasurementModeCollapsibleButton->setVisible(false);

  this->label_5->setVisible(false);
  this->costFunctionComboBox->setVisible(false);
  this->label_6->setVisible(false);
  this->scalarFunctionPrefixLabel->setVisible(false);
  this->scalarFunctionSuffixLabel->setVisible(false);
  this->scalarFunctionLineEdit->setVisible(false);

  this->resampleCurveOutputNodeLabel->setVisible(false);
  this->resampleCurveOutputNodeSelector->setVisible(false);
  this->resampleCurveConstraintNodeLabel->setVisible(false);
  this->resampleCurveConstraintNodeSelector->setVisible(false);
  this->AdvancedOptionCollapsibleGroupBox->setVisible(false);

  this->modelNodeSelector->setRemoveEnabled(false);
  this->modelNodeSelector->setAddEnabled(false);
  ...
  this->activeMarkupTableWidget->setAlternatingRowColors(false);

  this->activeMarkupTableWidget->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
  ...
  this->activeMarkupTableWidget->setColumnWidth(this->columnIndex("名称"), 60);
  this->activeMarkupTableWidget->setColumnWidth(this->columnIndex("描述"), 120);
  ...
  this->activeMarkupTableWidget->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
  this->activeMarkupTableWidget->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
  this->activeMarkupTableWidget->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
  ...
  for (int curveType = 0; curveType < vtkCurveGenerator::CURVE_TYPE_LAST; ++curveType)
    {
      QString curveTypeName = qSlicerMarkupsModuleWidgetPrivate::getCurveTypeAsHumanReadableString(curveType);
      if (curveTypeName == QString("Kochanek spline"))
      {
          continue;
      }
      if (curveTypeName == QString("Polynomial"))
      {
          continue;
      }
      this->curveTypeComboBox->addItem(curveTypeName, curveType);
    }
...
void qSlicerMarkupsModuleWidget::updateWidgetFromMRML()
{
  ...
  if (transformNodeID == nullptr)
    {
    d->transformedCoordinatesCheckBox->setText("变形");
    }
  else
    {
...
void qSlicerMarkupsModuleWidget::updateRow(int m)
{
  ...
  if (markupsNode->GetNthControlPointVisibility(m))
   ...
   // name
   int nameIndex = d->columnIndex("名称");
   ...
   // description
   int descriptionIndex = d->columnIndex("描述");
...
void qSlicerMarkupsModuleWidget::onHideCoordinateColumnsToggled(bool checked)
{
  ...
  if (!checked)
    ...
    d->activeMarkupTableWidget->setColumnWidth(d->columnIndex("名称"), 60);
    d->activeMarkupTableWidget->setColumnWidth(d->columnIndex("描述"), 120);
    }
  else
    {
    // expand the name and description columns
    d->activeMarkupTableWidget->setColumnWidth(d->columnIndex("名称"), 120);
    d->activeMarkupTableWidget->setColumnWidth(d->columnIndex("描述"), 240);
    }
}
