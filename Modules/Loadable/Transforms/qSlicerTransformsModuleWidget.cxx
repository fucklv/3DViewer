...
void qSlicerTransformsModuleWidget::setup()
{
  ...
  QAction* NewLinearTrans = new QAction(QString("Create new LinearTransform"));
  QAction* NewBSplineTrans = new QAction(QString("Create new BSplineTransform"));
  QAction* NewGridTrans = new QAction(QString("Create new GridTransform"));
  QAction* NewTransform = new QAction(QString("Create new Transform"));
  connect(NewLinearTrans, SIGNAL(triggered()), this, SLOT(onCreateNewTFButtonTriggered()));
  connect(NewBSplineTrans, SIGNAL(triggered()), this, SLOT(onCreateNewTFButtonTriggered()));
  connect(NewGridTrans, SIGNAL(triggered()), this, SLOT(onCreateNewTFButtonTriggered()));
  connect(NewTransform, SIGNAL(triggered()), this, SLOT(onCreateNewTFButtonTriggered()));

  CreateNewMenu->addAction(NewLinearTrans);
  CreateNewMenu->addAction(NewBSplineTrans);
  CreateNewMenu->addAction(NewGridTrans);
  CreateNewMenu->addAction(NewTransform);

  CreateNewMenu->setStyleSheet(
      "QMenu{"\
      "width: 230;"\
      "color: #FFFFFF;"\
      "background: #3c3c5a;}"\
  ...
  d->CreateNewTFButton->setMenu(CreateNewMenu);

  //as ²Ëµ¥
  QMenu* CreateNewASMenu = new QMenu(this);
  QAction* AsLinearTrans = new QAction(QString("Create new LinearTransform as..."));
  QAction* AsBSplineTrans = new QAction(QString("Create new BSplineTransform as..."));
  QAction* AsGridTrans = new QAction(QString("Create new GridTransform as..."));
  QAction* AsTransform = new QAction(QString("Create new Transform as..."));
  connect(AsLinearTrans, SIGNAL(triggered()), this, SLOT(onCreateNewASTFButtonTriggered()));
  connect(AsBSplineTrans, SIGNAL(triggered()), this, SLOT(onCreateNewASTFButtonTriggered()));
  connect(AsGridTrans, SIGNAL(triggered()), this, SLOT(onCreateNewASTFButtonTriggered()));
  connect(AsTransform, SIGNAL(triggered()), this, SLOT(onCreateNewASTFButtonTriggered()));

  CreateNewASMenu->addAction(AsLinearTrans);
  CreateNewASMenu->addAction(AsBSplineTrans);
  CreateNewASMenu->addAction(AsGridTrans);
  CreateNewASMenu->addAction(AsTransform);

  CreateNewASMenu->setStyleSheet(
      "QMenu{"\
      "width: 230;"\
      "color: #FFFFFF;"\
      "background: #3c3c5a;}"\
  ...
  d->CreateNewASTFButton->setMenu(CreateNewASMenu);

  d->ComboboxTfButtonWidget->setVisible(false);

  d->InfoCollapsibleWidget->setVisible(false);
  d->DisplayCollapsibleButton->setVisible(false);
  d->ConvertCollapsibleButton->setVisible(false);
  d->TranslateFirstToolButton->setVisible(false);
  d->CopyTransformToolButton->setVisible(false);
  d->PasteTransformToolButton->setVisible(false);
  ...
  d->TransformNodeSelector->setRemoveEnabled(false);
  //d->TransformNodeSelector->setRenameEnabled(false);
  ...
  d->TransformToolButton->setIcon(QIcon(":/Icons/rightIcon.png"));

  QIcon leftIcon =
    QApplication::style()->standardIcon(QStyle::SP_ArrowLeft);
  d->UntransformToolButton->setIcon(QIcon(":/Icons/leftIcon.png"));
...
void qSlicerTransformsModuleWidget::on_RenameCurrentTFButton_clicked()
{
    Q_D(qSlicerTransformsModuleWidget);

    d->TransformNodeSelector->renameCurrentNode();
}

void qSlicerTransformsModuleWidget::onCreateNewTFButtonTriggered()
{
    Q_D(qSlicerTransformsModuleWidget);
    
    QAction* action = (QAction*)sender();
    QString CreateString = action->text();
    if (!CreateString.isEmpty())
    {
        d->TransformNodeSelector->createNewAndAsNode(CreateString);
    }
}

void qSlicerTransformsModuleWidget::onCreateNewASTFButtonTriggered()
{
    Q_D(qSlicerTransformsModuleWidget);
    QAction* action = (QAction*)sender();
    QString CreateString = action->text();
...
void qSlicerTransformsModuleWidget::on_DleCurrentTFButton_clicked()
{
    Q_D(qSlicerTransformsModuleWidget);

    d->TransformNodeSelector->removeCurrentNode();
}

void qSlicerTransformsModuleWidget::on_EditVolumeTFButton_clicked()
{
    Q_D(qSlicerTransformsModuleWidget);

    d->ConvertReferenceVolumeNodeComboBox->renameCurrentNode();
}

void qSlicerTransformsModuleWidget::on_DeleteVolumeTFButton_clicked()
{
    Q_D(qSlicerTransformsModuleWidget);

    d->ConvertReferenceVolumeNodeComboBox->removeCurrentNode();
}
