  ...
  , BrushPixelModeCheckbox(nullptr)
{
  ...
  this->PaintIcon = QIcon(":/Icons/PainSg1.png");
...
void qSlicerSegmentEditorPaintEffect::setupOptionsFrame()
{
  ...
  QLabel* PainImageLabel = new QLabel();
  QLabel* PainTextLabel = new QLabel();
  PainTextLabel->setStyleSheet("QLabel{font: 12pt 'Microsoft YaHei';}");
  PainImageLabel->setStyleSheet("QLabel{min-width: 13; min-height: 13; max-width: 13; max-height: 13;background-image: url(:/Icons/PainSg1.png);}");
  PainTextLabel->setText(" 画图");

  if (this->m_AlwaysErase)
  {
      PainImageLabel->setStyleSheet("QLabel{min-width: 13; min-height: 13; max-width: 13; max-height: 13;background-image: url(:/Icons/RubberSg1.png);}");
      PainTextLabel->setText(" 橡皮擦");
  }

  iconLayout->addWidget(PainImageLabel);
  iconLayout->addWidget(PainTextLabel);
  iconLayout->addStretch();
  this->addOptionsWidget(iconLayout);
  iconLayout->setContentsMargins(0, 0, 0, 10);
  ...
  diameterLabel->setStyleSheet("QLabel{font: 12pt 'Microsoft YaHei';}");
  ...
  //cxc 2020-9-22
  d->BrushDiameterFrame->setStyleSheet("QToolButton{background: #1a1a26; border: 1px solid #5c5c74;font: 12pt 'Microsoft YaHei';}QToolButton:hover {background-color: #5C5C74;}");
  d->BrushDiameterSpinBox->setFixedWidth(100);
  ...
  foreach (int diameter, quickDiameters)
    ...
    //quickDiameterButton->setToolTip("将画笔大小设置为屏幕大小的百分比或固定长度");
    quickDiameterButton->setStyleSheet("QToolButton{ min-width: 18; min-height: 18; max-width: 18; max-height: 18; font: 10pt 'Microsoft YaHei';}");
  ...
  d->BrushDiameterRelativeToggle->setIcon(QIcon(":/Icons/ChangeIcon.png"));
  d->BrushDiameterRelativeToggle->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
  //d->BrushDiameterRelativeToggle->setToolTip("在窗口大小百分比/绝对大小（以毫米为单位）之间切换直径快速设置按钮");
  d->BrushDiameterRelativeToggle->setStyleSheet("QToolButton{ min-width: 40; min-height: 18; max-width: 40; max-height: 18; font: 12pt 'Microsoft YaHei';}");
  ...
  //d->BrushSphereCheckbox->setToolTip("使用3D球形笔刷而不是2D圆形笔刷");
  d->BrushSphereCheckbox->setStyleSheet("QCheckBox{font: 12pt 'Microsoft YaHei';}");
  hbox->addWidget(d->BrushSphereCheckbox);

  d->EditIn3DViewsCheckbox = new QCheckBox("Edit in 3D views");
  //d->EditIn3DViewsCheckbox->setToolTip("允许在3D视图中绘画。 如果启用，则在3D视图中单击并拖动将在视图中绘制而不是旋转视图。");
  d->EditIn3DViewsCheckbox->setStyleSheet("QCheckBox{font: 12pt 'Microsoft YaHei';}");
  hbox->addWidget(d->EditIn3DViewsCheckbox);

  d->ColorSmudgeCheckbox = new QCheckBox("Color smudge");
  //d->ColorSmudgeCheckbox->setToolTip("通过采样笔刷笔划开始的像素位置来选择片段。如果画笔笔划在一个空白区域开始，则画笔会从所选线段中删除高亮区域。");
  d->ColorSmudgeCheckbox->setStyleSheet("QCheckBox{font: 12pt 'Microsoft YaHei';}");
  d->ColorSmudgeCheckbox->setVisible(false);
  if (!this->m_AlwaysErase)
    {
  ...
  //d->EraseAllSegmentsCheckbox->setToolTip("如果未选中，则从所有线段中删除高空区域。如果未选中，则仅从所选段中擦除区域。");
  d->EraseAllSegmentsCheckbox->setStyleSheet("QCheckBox{font: 12pt 'Microsoft YaHei';}");
  if (this->m_AlwaysErase)
    {
