...
void qMRMLSegmentationDisplayNodeWidgetPrivate::init()
{
  ...
  this->label_SliceFill->setVisible(false);
  this->label_Visible->setVisible(false);
  this->checkBox_Visible->setVisible(false);
  this->checkBox_VisibilitySliceFill->setVisible(false);
  this->SliderWidget_OpacitySliceFill->setVisible(false);
  this->label_SliceOutline->setVisible(false);
  this->checkBox_VisibilitySliceOutline->setVisible(false);
  this->SliderWidget_OpacitySliceOutline->setVisible(false);
  this->checkBox_Visibility3D->setVisible(false);
  this->CollapsibleGroupBox_Advanced->setVisible(false);
...
void qMRMLSegmentationDisplayNodeWidget::updateSelectedSegmentSection()
{
  ...
  if (!d->SegmentationDisplayNode || d->SelectedSegmentID.isEmpty())
    ...
    d->labelGroupBoxTitle->setText("Ñ¡ÔñSement£ºnone");
    return;
    }
  ...
  QString newTitle = QString("Ñ¡ÔñSement£º %1").arg(selectedSegment->GetName());
  //d->groupBox_SelectedSegment->setTitle(newTitle);
  d->labelGroupBoxTitle->setText(newTitle);
}
