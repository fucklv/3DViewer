  ...
  , RotateWarningInNodeSelectorLayout(true)
{
  ...
  this->EffectNameOrder
    // Local painting
    << "Paint" << "Erase" << "Scissors" /*<< "Level tracing"*/ << "Smoothing" /*<< "Draw"*/
    // Global processing
    << "Grow from seeds" /*<< "Fill between slices"*/ << "Threshold" << "Margin" << "Hollow"
...
void qMRMLSegmentEditorWidgetPrivate::init()
{
 ...
 /* this->OverwriteModeComboBox->addItem(qMRMLSegmentEditorWidget::tr("Overwrite all"), vtkMRMLSegmentEditorNode::OverwriteAllSegments);
  ...
  segmentationsButtonMenu->setStyleSheet(
      "QMenu::item{"\
      "color: #FFFFFF;}"\
      "QMenu::item:selected{"\
      "background: #6987d5;}"\
  ...
  this->SurfaceSmoothingSlider->setValue(0.4);
  ...
  surfaceSmoothingFactorMenu->menuAction()->setVisible(false);
  ...
  QObject::connect(this->SegmentationNodeComboBox,
      SIGNAL(nodeAdded(vtkMRMLNode*)),q,SLOT(SetMrmlChange(vtkMRMLNode*)));
 // QObject::connect(this->MasterVolumeNodeComboBox, SIGNAL(nodeAdded(vtkMRMLNode*)),
   //   q, SLOT(SetMasterVolumeChange(vtkMRMLNode*)));
  ...
  this->SegmentsTableView->setOpacityColumnVisible(true);
  this->SegmentsTableView->setFixedSize(this->SegmentsTableView->width(), this->SegmentsTableView->height());
  ...
  this->MaskWidgetLayout->setVisible(false);
  this->ComBoBoxButtonSegmentWidget->setVisible(false);
  this->ComBoBoxButtonMasterWidget->setVisible(false);
  this->CreateNewSgButton->setVisible(this);
  ...
  effectsGroupLayout->setHorizontalSpacing(4);
  effectsGroupLayout->setVerticalSpacing(4);

  effectsGroupLayout->setContentsMargins(4, 4, 4, 4);
  effectsGroupLayout->setSpacing(4);
  effectsGroupLayout->setAlignItems(false);
  effectsGroupLayout->setAlignment(Qt::AlignVCenter | Qt::AlignHCenter);
  effectsGroupLayout->setPreferredExpandingDirections(Qt::Vertical);
  //effectsGroupLayout->setSizeConstraint(QLayout::SetFixedSize);
  ...
  q->setSwitchToSegmentationsButtonVisible(false); 
  ...
  this->MasterVolumeIntensityMaskRangeLabel->setVisible(false);
  this->MasterVolumeIntensityMaskCheckBox->setVisible(false);
  this->OverwriteModeLabel->setVisible(false);
  this->OverwriteModeComboBox->setVisible(false);
  //this->toolTabelWidget->setVisible(false);
  ...
  this->MasterVolumeNodeComboBox->setRemoveEnabled(false);
  this->MasterVolumeNodeComboBox->setRenameEnabled(false);
}
...
QToolButton* qMRMLSegmentEditorWidgetPrivate::toolButton(qSlicerSegmentEditorAbstractEffect* effect)
{
  ...
  foreach(QAbstractButton* effectButton, effectButtons)
    ...
    effectButton->setStyleSheet("QToolButton{min-width: 32; min-height: 26; max-width: 32; max-height: 26; border: none;}"\
        "QToolButton:hover{background-color: #3c3c5a; border: 1px solid #5c5c74; }QToolButton:checked {background-color: #3c3c5a; border: 1px solid #5c5c74;}");
...
void qMRMLSegmentEditorWidget::updateEffectList()
{
  ...
  if (d->toolButton(nullptr) == nullptr)
    ...
    effectButton->setIcon(QIcon(":Icons/NotSelected1.png"));
    effectButton->setIconSize(QSize(13, 13));
    effectButton->setText("None");
    effectButton->setToolTip("未选择");
  ...
  d->EffectsGroupBox->layout()->setContentsMargins(0, 0, 0, 0);
  d->EffectsGroupBox->layout()->setSpacing(0);
  ...
  foreach(qSlicerSegmentEditorAbstractEffect* effect, addedEffects)
    ...
    effectButton->setToolTip(effect->toolTipName());
    effectButton->setSizePolicy(QSizePolicy::MinimumExpanding, effectButton->sizePolicy().verticalPolicy());
    effectButton->setProperty("Effect", QVariant::fromValue<QObject*>(effect));
    d->EffectButtonGroup.addButton(effectButton);
    auto i = d->EffectButtonGroup.buttons();
...
void qMRMLSegmentEditorWidget::updateMaskingSection()
{
  ...
  if (d->MaskingGroupBox->isVisible())
  {
      if (d->MasterVolumeIntensityMaskCheckBox->isChecked())
      {
          this->onMasterVolumeIntensityMaskChecked(true);
      }
  }
...
//-----------------------------------------------------------------------------kyo
void qMRMLSegmentEditorWidget::SetMrmlChange(vtkMRMLNode* node)
{
    Q_D(qMRMLSegmentEditorWidget);
    auto name = node->GetName();
    if (strcmp(name, "Segmentation_1") == 0)
    {
        d_ptr->SegmentationNodeComboBox->setCurrentNode(node);
    }
}

//kyo
void qMRMLSegmentEditorWidget::SetMasterVolumeChange(bool* b)
{
    Q_D(qMRMLSegmentEditorWidget);
    if (*b)
    {
        ...
        if (!nodeList.isEmpty())
        {
            emit this->d_ptr->MasterVolumeNodeComboBox->currentNodeChanged(nodeList[0]);
            this->d_ptr->MasterVolumeNodeLabel->setVisible(true);
            this->d_ptr->MasterVolumeNodeComboBox->setVisible(true);
            auto btns = d->EffectButtonGroup.buttons();
            for (int i = 0; i < btns.size(); i++)
            {
                auto str = btns[i]->text();
                QByteArray ba = str.toLatin1(); // must
                if (strcmp(ba.data(), "Scissors") == 0)
                {
                    onEffectButtonClicked(btns[i]);
                    qSlicerSegmentEditorScissorsEffect* p =  (qSlicerSegmentEditorScissorsEffect*)this->activeEffect();
                    p->setOperation(2);
                }
            }
        }
    ...
    else
    {
        ...
        for (int i = 0; i < btns.size(); i++)
        {
            ...
            if (strcmp(ba.data(), "None") == 0)
            {
                this->d_ptr->MasterVolumeNodeLabel->setVisible(false);
                this->d_ptr->MasterVolumeNodeComboBox->setVisible(false);
                onEffectButtonClicked(btns[i]);
            }
        }
    }
...
{
  Q_D(qMRMLSegmentEditorWidget);
  QString buuttonText = button->text();
  ...
  d->EffectsOptionsFrame->layout()->update();
}
...
void qMRMLSegmentEditorWidget::onAddSegment()
{
  ...
  //kyo
  auto node = d->SegmentsTableView->selectedSegmentIDs();
  if (node.size() > 0)
  {
      auto s = segmentationNode->GetSegmentation()->GetSegment(node[0].toStdString());
      auto name = s->GetName(); 
  }
...
void qMRMLSegmentEditorWidget::onCreateSurfaceToggled(bool on)
{
  ...
  if (on)
  {
      d->Show3DButton->setStyleSheet("QPushButton{"\
          "background: #6987d5;"\
...
void qMRMLSegmentEditorWidget::onMasterVolumeIntensityMaskChecked(bool checked)
{
  ...
  d->MaskWidgetLayout->setVisible(checked);
...
void qMRMLSegmentEditorWidget::on_RenameCurrentSgButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    d->SegmentationNodeComboBox->renameCurrentNode();
}

void qMRMLSegmentEditorWidget::on_CreateNewSgButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    d->SegmentationNodeComboBox->createNewAndAsNode("Create new Segmentation");
}

void qMRMLSegmentEditorWidget::on_CreateNewSgAsButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    d->SegmentationNodeComboBox->createNewAndAsNode("Create new Segmentation as...");
}

void qMRMLSegmentEditorWidget::on_DleCurrentSgButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    d->SegmentationNodeComboBox->removeCurrentNode();
}

void qMRMLSegmentEditorWidget::on_RenameCurrentVleButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    d->MasterVolumeNodeComboBox->renameCurrentNode();
}

void qMRMLSegmentEditorWidget::on_SelectMasterVleEditingButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
    this->setMasterVolumeNodeID("Select master volume to enable editing");
}

void qMRMLSegmentEditorWidget::on_DleCurrentVleButton_clicked()
{
    Q_D(qMRMLSegmentEditorWidget);
