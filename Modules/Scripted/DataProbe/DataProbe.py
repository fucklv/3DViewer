...
class DataProbeInfoWidget(object):
  ...
  def processEvent(self,observee,event):
    ...
    if hasattr(color, 'name'):
      self.viewerColor.setStyleSheet('QLabel {font: 10pt SimHei; background-color : %s}' % color.name())
  ...
  def generateViewDescription(self, xyz, ras, sliceNode, sliceLogic):
    ...
    return \
      "{orient: >8s} Sp:{spacing:s}" \
      .format(orient=sliceNode.GetOrientationString(),
              spacing=spacing
              )
  ...
  def _createSmall(self):
    ...
    self.viewerFrame.layout().addStretch()
    self.viewerFrame.layout().addWidget(self.viewerColor)
    self.viewInfo = qt.QLabel()
    self.viewerFrame.layout().addWidget(self.viewInfo)
    self.viewerFrame.layout().setContentsMargins(0,0,0,0)
    
    self.viewInfo.setStyleSheet("QLabel{font: 10pt 'SimHei';}")
