  ...
  , q_ptr(&object)
{
  ...
  this->p_ViewLabel = nullptr;
  this->m_barVisible = false;
...
void qMRMLViewControllerBarPrivate::init()
{
  ...
  this->p_ViewLabel = new QLabel(q);
  this->p_ViewLabel->setObjectName("p_ViewLabel");
  this->p_ViewLabel->setAlignment(Qt::AlignHCenter | Qt::AlignCenter);
  this->p_ViewLabel->setMinimumWidth(this->p_ViewLabel->fontMetrics().width("XX"));
  this->p_ViewLabel->setAutoFillBackground(true);
  this->BarLayout->addWidget(this->p_ViewLabel);
  ...
  this->BarWidget->setStyleSheet("background-color: rgb(89,89,122)");
}
...
{
    if (this->PopupWidget != nullptr)
    {
        QWidget* widget = qobject_cast<QWidget*>(this->PinButton->topLevelWidget()->topLevelWidget()->topLevelWidget()->topLevelWidget());
        HWND hwnd = reinterpret_cast<HWND>(widget->winId());
        if(RecordInformationTool::getInstance()->getPopWidgetDICOMVisble())
        {
            if (RecordInformationTool::getInstance()->getWindowVisible())
            {
                this->PopupWidget->setVisible(false);
            }
            
            if (!RecordInformationTool::getInstance()->getWindowVisible())
            {
                if (RecordInformationTool::getInstance()->getWindowLayout() == 3)
                {
                    this->PopupWidget->setVisible(true);
                }
                else if (RecordInformationTool::getInstance()->getWindowLayout() == 4)
                {
                    //this->PopupWidget->setVisible(true);
                }
                else if ((RecordInformationTool::getInstance()->getWindowLayout() == 6)
                    && !RecordInformationTool::getInstance()->getWindowLayoutChange())
                {
                    this->PopupWidget->setVisible(true);
                    RecordInformationTool::getInstance()->setWindowLayoutChange(true);
                }
                else if ((RecordInformationTool::getInstance()->getWindowLayout() == 7)
                    && !RecordInformationTool::getInstance()->getWindowLayoutChange())
                {
                    this->PopupWidget->setVisible(true);
                    RecordInformationTool::getInstance()->setWindowLayoutChange(true);
                }
                else if ((RecordInformationTool::getInstance()->getWindowLayout() == 8)
                    && !RecordInformationTool::getInstance()->getWindowLayoutChange())
                {
                    this->PopupWidget->setVisible(true);
                    RecordInformationTool::getInstance()->setWindowLayoutChange(true);
...
void qMRMLViewControllerBar::paintEvent(QPaintEvent*)
{
    Q_D(qMRMLViewControllerBar);
    if (!isHidden() && d->m_barVisible)
    {
        QWidget* widget = qobject_cast<QWidget*>(d->PinButton->nativeParentWidget()->nativeParentWidget());
        if (widget != nullptr)
        {
            if (!widget->isHidden())
            {
                d->PopupWidget->pinPopup(true);
                d->m_barVisible = false;
            }
        }
    }
}
