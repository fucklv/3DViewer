...
void qMRMLNodeComboBoxPrivate::updateActionItems(bool resetRootIndex)
{
  ...
  // --------------------------------------------------------------------------
  // 清除Transform的TransformNodeSelector多余选项
  // --------------------------------------------------------------------------
  QStringList deleteList = extraItems.filter(QString("Create new BSplineTransform as..."));
  if (deleteList == QStringList("Create new BSplineTransform as..."))
  {
      extraItems.clear();
      extraItems.append("separator");
      //extraItems.append("Create new Transform");
      extraItems.append("Create new Transform as...");
  }
  // --------------------------------------------------------------------------
  // 清除Gradient Anisotropic Diffusion IO的Output Volume多余选项
  // --------------------------------------------------------------------------
  QStringList deleteList1 = extraItems.filter(QString("Create new LabelMapVolume as..."));
  if (deleteList1 == QStringList("Create new LabelMapVolume as..."))
  {
      extraItems.clear();
      extraItems.append("separator");
      extraItems.append("Create new Volume");
      extraItems.append("Create new Volume as...");
  }
  // --------------------------------------------------------------------------
...
void qMRMLNodeComboBox::renameCurrentNode()
{
 ...
 /* QString newName = QInputDialog::getText(
  ...
  inputDialog->setWindowTitle("\351\207\215\345\221\275\345\220\215");
  inputDialog->setLabelText("\346\226\207\344\273\266\345\220\215:");
  inputDialog->setTextValue("name");
  inputDialog->setOkButtonText("\347\241\256\345\256\232");
  inputDialog->setCancelButtonText("\345\217\226\346\266\210");
...
void qMRMLNodeComboBox::hideComboxItem()
{
    Q_D(qMRMLNodeComboBox);
}

void qMRMLNodeComboBox::createNewAndAsNode(const QString& name)
{
    Q_D(qMRMLNodeComboBox);
