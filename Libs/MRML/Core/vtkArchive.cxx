...
bool vtkArchive::VeiwerZip(const char* zipFileName, const char* directoryToZip, std::vector<std::string>& files)
{
  ...
  // only support the libarchive version 3.0 +
    ...
    //kyo
    auto PassRes = archive_write_set_passphrase(zipArchive, "zcwl888999");
    if (PassRes != ARCHIVE_OK)
    {
        auto cool = archive_error_string(zipArchive);
        vtkArchiveTools::Error("zip:", cool);
        return false;
    }

    archive_write_set_format_zip(zipArchive);
    archive_write_set_format_option(zipArchive, "zip", "compression", compression_type.c_str());
    archive_write_set_format_option(zipArchive, "zip", "encryption", "aes256");
   ...
   /* dirEntry = archive_entry_new();
    ...
    {
        vtkArchiveTools::Message("Zip: adding:", (*sit).c_str());
        auto stringfileName = directoryToZip+ std::string("/") + (*sit);
...
bool vtkArchive::UnZip(const char* zipFileName, const char* destinationDirectory)
{
  ...
  //kyo
  auto PassRes = archive_read_add_passphrase(zipArchive, "zcwl888999");
  if (PassRes != ARCHIVE_OK)
  {
      auto cool = archive_error_string(zipArchive);
      vtkArchiveTools::Error("Unzip:", cool);
      return false;
  }
  ...
  for (;;)
    ...
    //kyo
    auto ifEncrypted = archive_read_has_encrypted_entries(zipArchive);
    if (ifEncrypted != 1){
        break;
    }
